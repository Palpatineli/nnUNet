...
from nnunetv2.training.lr_scheduler.cosine import LinearWarmupCosineAnnealingLR
...
from nnunetv2.utilities.utils import get_param_groups

# belongs in a nnUNetTrainer obj, so use the same signature
def get_training_transforms(patch_size: np.ndarray | Tuple[int],
    ...
    tr_transforms.append(SpatialTransform(
        tuple(patch_size_spatial), patch_center_dist_from_border=max(patch_size_spatial) // 2,
    ...
    tr_transforms = ComposeTransforms(tr_transforms)
...
def get_validation_transforms(deep_supervision_scales: Union[List, Tuple],
    ...
    val_transforms = ComposeTransforms(val_transforms)
...
def main():
    ...
    param_groups = get_param_groups(model_without_ddp, no_weight_decay_keys={'cls_token', 'pos_embed', 'mask_token', 'gamma'})
    opt_clz = {
