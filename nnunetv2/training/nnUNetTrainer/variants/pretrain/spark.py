...
from dynamic_network_architectures.architectures.sparsify import MaskedModule
...
class SparK(nn.Module):
    """
       ...
       """
    def __init__(
            self, masked_encoder: MaskedModule, dense_decoder: LightDecoder,
            mask_ratio=0.6, densify_norm='in', sbn=False
    ):
        super().__init__()
        input_size, downsample_ratio = masked_encoder.input_size, masked_encoder.downsample_ratio
        ...
        self.sparse_encoder = masked_encoder
        self.dense_decoder = dense_decoder

        self.sbn = sbn
        self.hierarchy = len(masked_encoder.enc_feat_map_chs)
