...
def linear_warmup_decay(warmup_steps: int, total_steps: int, cosine: bool = True,
                        linear: bool = False) -> Callable[[int], float]:
    """Linear warmup for warmup_steps, optionally with cosine annealing or linear decay to 0 at total_steps."""
    decay_steps = total_steps - warmup_steps
    if linear:
        if cosine:
            raise ValueError("can only pick one decay option")
        else:
            def decay(step: int) -> float:  # type: ignore
