...
def get_patch_size(spacing: np.ndarray | Tuple[float] | List[float], initial_patch_size: List[int], min_feature_map_size: int,
                   max_numpool: int) -> Tuple[np.ndarray, np.ndarray]:
    dim = len(initial_patch_size)
    num_pool_per_axis = [0] * dim
    current_spacing = deepcopy(list(spacing))
    current_size = deepcopy(list(initial_patch_size))
    while True:
        valid_axes_for_pool = [i for i in range(dim) if current_size[i] >= 2 * min_feature_map_size]
        if len(valid_axes_for_pool) < 1:
            break
        ...
        for v in valid_axes_for_pool:
            num_pool_per_axis[v] += 1
    must_be_divisible_by = get_shape_must_be_divisible_by(num_pool_per_axis)
    patch_size = pad_shape(initial_patch_size, must_be_divisible_by)
    return patch_size, must_be_divisible_by
